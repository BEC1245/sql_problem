...
select ename, empno from emp where deptno in(20, 30) order by ename desc;
...
select ename, sal, comm from emp where comm is not null and comm != 0 order by sal, comm desc;
...
select ename, job, sal from emp where (job = 'CLERK' OR JOB = 'SALESMAN') and not sal in(1600, 950, 1300); 
